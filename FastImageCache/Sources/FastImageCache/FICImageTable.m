...
@interface FICImageTable () {
    ...
    NSMutableDictionary<NSNumber *, FICImageTableChunk *> *_chunkDictionary;
    NSCountedSet<FICImageTableChunk *> *_chunkSet;
    
    NSRecursiveLock *_lock;
    CFMutableDictionaryRef _indexNumbers;
    
    // Image table metadata
    NSMutableDictionary<NSString *, NSNumber *> *_indexMap;         // Key: entity UUID, value: integer index into the table file
    NSMutableDictionary<NSString *, NSString *> *_sourceImageMap;   // Key: entity UUID, value: source image UUID
    NSMutableIndexSet *_occupiedIndexes;
    NSMutableOrderedSet<NSString *> *_MRUEntries;
    NSCountedSet<NSString *> *_inUseEntries;
    ...
    if (entityUUID != nil && sourceImageUUID != nil && imageDrawingBlock != NULL) {
        ...
        if (newEntryIndex < _entryCount) {
            ...
            if (entryData != nil) {
                ...
                CGFloat scale = _imageFormat.scale;
        ...
        if (entryData != nil) {
            ...
            @synchronized(indexNumber) {
                if (entityUUIDIsCorrect == NO || sourceImageUUIDIsCorrect == NO) {
                    ...
                    if (imageRef != NULL) {
                        image = [[UIImage alloc] initWithCGImage:imageRef scale: _imageFormat.scale orientation:UIImageOrientationUp];
